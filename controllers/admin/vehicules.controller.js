...
const findAll = async (req, res) => {
    try {
        ...
        const defaultSortField = "VEHICULE_ID"
...
const updateVehicule = async (req, res) => {
    ...
    try {
        const { VEHICULE_ID } = req.params
        const {
            ID_MARQUE, ID_MODELE, PLAQUE, PROPRIETAIRE_ID,ANNEE_FABRICATION, NUMERO_CHASSIS, COULEUR, PHOTO
            , KILOMETRAGE,DATE_DEBUT_ASSURANCE, DATE_FIN_ASSURANCE, DATE_DEBUT_CONTROTECHNIK, DATE_FIN_CONTROTECHNIK
        } = req.body
        const files = req.files || {};
        const { FILE_ASSURANCE, FILE_CONTRO_TECHNIQUE, IMAGE_AVANT, IMAGE_ARRIERE, IMAGE_LATERALE_GAUCHE, IMAGE_LATERALE_DROITE
            , IMAGE_TABLEAU_DE_BORD, IMAGE_SIEGE_AVANT, IMAGE_SIEGE_ARRIERE } = files
        const data = { ...req.files, ...req.body };
        ...
        const datainsert = await Vehicules.create({
            ID_MARQUE, ID_MODELE, PLAQUE, PROPRIETAIRE_ID, USAGE_ID: 1, ANNEE_FABRICATION, NUMERO_CHASSIS, COULEUR, PHOTO
            , KILOMETRAGE, SHIFT_ID: 1, DATE_DEBUT_ASSURANCE, DATE_FIN_ASSURANCE, DATE_DEBUT_CONTROTECHNIK, DATE_FIN_CONTROTECHNIK,
            FILE_ASSURANCE: filename
                ? `${req.protocol}://${req.get("host")}/${IMAGES_DESTINATIONS.vehicule
                }/${filename.fileName}`
                : null,
