...
const findAll = async (req, res) => {
    try {
        ...
        const defaultSortField = "PROPRIETAIRE_ID"
        ...
        const resultss = await Promise.all(
            result.rows.map((driverObject) => {
                ...
                const vehiculnombre = vehicule.filter(
                    (allC) => proprietaire.PROPRIETAIRE_ID == allC.PROPRIETAIRE_ID
                );
    ...
    try {
        ...
        if (PHOTO_PASSPORT) {
            const { fileInfo: fileInfo_1, thumbInfo: thumbInfo_1 } =
                await propietaireUpload.upload(PHOTO_PASSPORT, false);
            filename1 = fileInfo_1;
        }
        if (FILE_CNI_PASSPORT) {
            const { fileInfo: fileInfo_2, thumbInfo: thumbInfo_2 } =
                await propietaireUpload.upload(FILE_CNI_PASSPORT, false);
            filename2 = fileInfo_2;
        }
        if (LOGO) {
            const { fileInfo: fileInfo_3, thumbInfo: thumbInfo_3 } =
                await propietaireUpload.upload(LOGO, false);
            filename3 = fileInfo_3;
        }
        if (FILE_NIF) {
            const { fileInfo: fileInfo_4, thumbInfo: thumbInfo_4 } =
                await propietaireUpload.upload(FILE_NIF, false);
            filename4 = fileInfo_4;
        }
        if (FILE_RC) {
            const { fileInfo: fileInfo_5, thumbInfo: thumbInfo_5 } =
                await propietaireUpload.upload(FILE_RC, false);
        ...
        const datainsert = await Proprietaire.create({
            ...
            PHOTO_PASSPORT: filename1
                ? `${req.protocol}://${req.get("host")}/${IMAGES_DESTINATIONS.Proprietairepassport
                }/${filename1.fileName}`
                : null,
    ...
    try {
        ...
        const datainsert = await Proprietaire.create({
            ...
            PHOTO_PASSPORT: filename1
                ...
                : null,
            FILE_CNI_PASSPORT: filename2
                ? `${req.protocol}://${req.get("host")}/${IMAGES_DESTINATIONS.Proprietairefilecni
                }/${filename2.fileName}`
                : null,
            LOGO: filename3
                ? `${req.protocol}://${req.get("host")}/${IMAGES_DESTINATIONS.Proprietairelogo
                }/${filename3.fileName}`
                : null,
            FILE_NIF: filename4
                ? `${req.protocol}://${req.get("host")}/${IMAGES_DESTINATIONS.Proprietairefilenif
                }/${filename4.fileName}`
                : null,
            FILE_RC: filename5
                ? `${req.protocol}://${req.get("host")}/${IMAGES_DESTINATIONS.Proprietairefilerc
                }/${filename5.fileName}`
                : null,
        }, {
            where: {
