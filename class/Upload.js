...
class Upload {
  ...
  async upload(image, withThumb = true) {
    ...
    const thumbName =
      path.parse(fileName).name + "_thumb" + path.extname(fileName);
    ...
    try {
      ...
      if (withThumb) {
        thumbInfo = await sharp(image.data)
          .resize(354, 221, { fit: "inside" })
          .toFormat("jpg")
          .toFile(thumbPath);
      }
      var fileInfo = await sharp(image.data)
        .resize(500)
  ...
  /**
   * Changer le chemin de destination
   * @param string path
