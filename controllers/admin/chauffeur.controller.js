...
const create_chauffeur = async (req, res) => {
   try {
      ...
      if (FILE_CARTE_IDENTITE) {
         const { fileInfo: fileInfo } = await vehiculeUploads.upload(FILE_CARTE_IDENTITE, false)
         filename = fileInfo
      }
      if (FILE_IDENTITE_COMPLETE) {
         const { fileInfo: fileInfo1 } = await vehiculeUploads.upload(FILE_IDENTITE_COMPLETE, false)
         filename1 = fileInfo1
      }
      if (FILE_CASIER_JUDICIAIRE) {
         const { fileInfo: fileInfo2 } = await vehiculeUploads.upload(FILE_CASIER_JUDICIAIRE, false)
         filename2 = fileInfo2
      }
      if (FILE_PERMIS) {
         const { fileInfo: fileInfo3 } = await vehiculeUploads.upload(FILE_PERMIS, false)
         filename3 = fileInfo3
      }
      if (PHOTO_PASSPORT) {
         const { fileInfo: fileInfo4 } = await vehiculeUploads.upload(PHOTO_PASSPORT, false)
         filename4 = fileInfo4
      }
      ...
      const datainsert = await chauffeur.create({
         ...
         FILE_IDENTITE_COMPLETE: filename1
            ? `${req.protocol}://${req.get("host")}/${IMAGES_DESTINATIONS.vehicule}/${filename1.fileName}`
            : null,
         FILE_CASIER_JUDICIAIRE: filename2
            ? `${req.protocol}://${req.get("host")}/${IMAGES_DESTINATIONS.vehicule}/${filename2.fileName}`
            : null,
...
const update_assureur = async (req, res) => {
   try {
      ...
      if (PHOTO_PASSPORT) {
         const { fileInfo: fileInfo4 } = await vehiculeUploads.upload(FILE_PERMIS, false)
         filename4 = fileInfo4
      }
      const datainsert = await chauffeur.update(
         {
            ...
            GENRE_ID: GENRE_ID,
            FILE_CARTE_IDENTITE: filename
               ? `${req.protocol}://${req.get("host")}/${IMAGES_DESTINATIONS.drivers}/${filename.fileName}`
               : null,
            FILE_IDENTITE_COMPLETE: filename1
               ? `${req.protocol}://${req.get("host")}/${IMAGES_DESTINATIONS.drivers}/${filename1.fileName}`
               : null,
            FILE_CASIER_JUDICIAIRE: filename2
               ? `${req.protocol}://${req.get("host")}/${IMAGES_DESTINATIONS.drivers}/${filename2.fileName}`
               : null,
            FILE_PERMIS: filename3
               ? `${req.protocol}://${req.get("host")}/${IMAGES_DESTINATIONS.drivers}/${filename3.fileName}`
               : null,
            PHOTO_PASSPORT: filename4
               ? `${req.protocol}://${req.get("host")}/${IMAGES_DESTINATIONS.drivers}/${filename4.fileName}`
               : null,
         },
         {
...
const change_status = async (req, res) => {
   try {
      const { CHAUFFEUR_ID } = req.params
      const UtiObject = await chauffeur.findByPk(CHAUFFEUR_ID, {
         attributes: ["CHAUFFEUR_ID", "IS_ACTIVE"],
      })
